    ...
    try {
      switch (event.type) {
        case 'customer.subscription.deleted': {
          ...
          if (!userId) {
            return NextResponse.json({ error: 'No userId found in subscription metadata' }, { status: 400 });
          }

          // Update user subscription in analytics backend
          const subscriptionResponse = await fetch(`${ANALYTICS_URL}/api/users/${userId}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (!subscriptionResponse.ok) {
            ...
            throw new Error(data.detail || 'Failed to update subscription status');
          }

          // Reset user credits and minutes to 0
          const creditsFormData = new FormData();
          creditsFormData.append('user_id', userId);
          creditsFormData.append('amount', '0');
          creditsFormData.append('stripe_payment_id', subscription.id);
          creditsFormData.append('minutes_balance', '0');

          const creditsResponse = await fetch(`${ANALYTICS_URL}/api/credits/add`, {
            method: 'POST',
            body: creditsFormData
          });

          if (!creditsResponse.ok) {
            const data = await creditsResponse.json();
            throw new Error(data.detail || 'Failed to reset credits and minutes');
          }

          return NextResponse.json({ 
            message: 'Subscription expired and balances reset successfully'
          });
        }
